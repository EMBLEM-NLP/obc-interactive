...
OLD = os.environ.get("OBC_V9_PDF", "")
SRC = os.environ.get("OBC_SRC_V1", "")

# fetch-only input: say so plainly rather than trace back on FileNotFoundError
for _v in ("OBC_SRC_V1", "OBC_V9_PDF"):
    if not os.environ.get(_v) or not os.path.exists(os.environ[_v]):
        ...
        sys.exit(1)
LAB = os.environ.get("OBC_LABELS", os.path.join(_PKG, "verify/data/v1/labels.json"))
nodes = {}
for line in gzip.open("out/docgraph.jsonl.gz", "rt"):
    r = json.loads(line)
    if not r.get("_meta"):
        nodes[r["id"]] = r
a, b, s = pymupdf.open(NEW), pymupdf.open(OLD), pymupdf.open(SRC)
...
try:
    v2 = pymupdf.open(os.environ.get("OBC_BUILT_V2", os.path.join(_PKG, "pdf/301881_built_from_model.pdf")))
except Exception:
    v2 = None
fails = []
...
def kinds(d):
    ...
    for p in d:
        for l in p.get_links():
            c[l["kind"]] += 1
    return c
ka, kb, ks = kinds(a), kinds(b), kinds(s)
...
for pg in MASTER + TOC:
    ...
    nb = len([l for l in b[pg-1].get_links() if l["kind"] in (1, 5)])
    if na < nb:
...
pb = {i + 1 for i in range(b.page_count) if any(l["kind"] == 1 for l in b[i].get_links())}
lost = sorted(pb - pa)
print(f"{'pages linked in v9, not here':<30}{len(lost):>9}   {lost[:10]}")
if lost: fails.append(f"{len(lost)} pages lost links relative to v9")
print(f"{'bookmarks':<30}{len(a.get_toc()):>9}{len(b.get_toc()):>9}")
if len(a.get_toc()) < len(b.get_toc()): fails.append("fewer bookmarks than v9")
