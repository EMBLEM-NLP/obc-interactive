...
for p, r in inv.items():
    lines = sorted(
        ((l["b"][1], "".join(s["t"] for s in l["s"]).strip())
         for b in r["blocks"] for l in b["l"]),
        key=lambda x: x[0],
    )
...
def build_path(ref):
    """Resolve stable package refs to the transient verifier/build location."""
    return os.path.join("out", "assets", os.path.basename(ref))
...
broken = [
    ...
    if not os.path.exists(build_path(ref)) or os.path.getsize(build_path(ref)) == 0
]
...
print(
    "uncaptioned assets (letterhead, signatures, preface art): "
