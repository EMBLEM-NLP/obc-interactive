# The package — 2024 Building Code Compendium, interactive edition

*Every count in this file is measured from the artifacts at package time by
`harden/gen_readme.py`, not typed. `FACTS.json` holds the same numbers as data.*

**This file describes the hydrated package** — a tree that holds all 29 derived
artifacts listed in `data-manifest.json`, or the BagIt bag built from one. It is
**not** a description of a git checkout: a clone carries none of those files.
`README.md` describes the checkout and says how to hydrate it.
...
*2026-09-08, track E9: this table moved here from `README.md` unchanged. The
mechanism that let it drift — a hand-maintained region outside the span
`gen_readme.py` rewrites — is unchanged too, and moving the table did not fix
it. What changed is that the file carrying it now says which tree it describes.*
